...
!function () { let e = document.createElement("canvas"); e.id = "starfield-bg", document.body.appendChild(e); let t = e.getContext("2d"); function i() { e.width = window.innerWidth, e.height = window.innerHeight } Object.assign(e.style, { position: "fixed", top: "0", left: "0", width: "100%", height: "100%", zIndex: "1", pointerEvents: "none" }), window.addEventListener("resize", i), i(); let n = []; for (let h = 0; h < 300; h++)n.push({ x: Math.random() * e.width, y: Math.random() * e.height, depth: 3 * Math.random() + 1, size: 2 * Math.random() + .5, opacity: .8 * Math.random() + .2 }); let d = 0, $ = 0, l = e.width / 2, o = e.height / 2; window.addEventListener("mousemov", e => { d = e.clientX, $ = e.clientY }), !function i() { t.clearRect(0, 0, e.width, e.height); let h = (d - l) / 100, a = ($ - o) / 100; for (let r = 0; r < n.length; r++){ let g = n[r]; g.x += h * (1 / g.depth), g.y += a * (1 / g.depth), g.x < 0 && (g.x = e.width), g.x > e.width && (g.x = 0), g.y < 0 && (g.y = e.height), g.y > e.height && (g.y = 0), t.fillStyle = `rgba(255, 255, 255, ${g.opacity})`, t.beginPath(), t.arc(g.x, g.y, g.size, 0, 2 * Math.PI), t.fill() } requestAnimationFrame(i) }() }();
