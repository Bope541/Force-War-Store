...
if (!document.querySelector("#cart-drawer")) {
  ...
  drawer.className = "fixed inset-0 hidden z-50 items-center justify-end bg-black/50";
  drawer.setAttribute("data-state", "closed");
  drawer.innerHTML = `
    <div class="w-full sm:w-[380px] h-full bg-background flex flex-col">
      <header class="flex items-center justify-between p-4 border-b">
        <h4 class="text-lg font-semibold">Seu Carrinho</h4>
        <button onclick="closeCart()" class="text-muted-foreground hover:text-foreground">
          <i data-lucide="x" class="w-5 h-5"></i>
        </button>
      </header>
      <main id="products-list" class="flex-1 overflow-y-auto p-4"></main>
      <footer id="cart-pricing" class="hidden items-center justify-between p-4 border-t">
        <p class="text-base font-medium">Total:</p>
        <span class="text-lg font-semibold">R$ 0,00</span>
      </footer>
    </div>`;
