...
class CartHelper {
  ...
  static async revalidate() {
    try {
      ...
      // Tenta buscar da API principal (se existir no futuro)
      try {
        const response = await fetch("https://api.forcewar.net/products");
        if (response.ok) {
          products = await response.json();
        } else {
          throw new Error("API principal fora do ar, usando fallback local.");
        }
      } catch (error) {
        console.warn("[ForceWar] API principal indisponível. Usando /assets/data/products.json");
        const localResponse = await fetch("assets/data/products.json");
      ...
      console.error("[ForceWar] Erro ao revalidar carrinho:", error);
    }
  }
